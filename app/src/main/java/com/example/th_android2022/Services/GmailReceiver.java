...
public class GmailReceiver {
    ...
    public GmailReceiver(AppCompatActivity activity){
        this.activity = activity;
        mCredential = GoogleAccountCredential.usingOAuth2(
                        activity.getApplicationContext(), Arrays.asList(SCOPES))
                .setBackOff(new ExponentialBackOff());

//        activity.registerForActivityResult(       //TODO move onActivityResult here
//
//        );
    }
    ...
    public void chooseAccount(View view) {
        Runnable runnable = () -> {
            try {
                activity.startActivityForResult(mCredential.newChooseAccountIntent(), RC_REQUEST_ACCOUNT_PICKER);
            } catch (Exception e) {
                e.printStackTrace();
            }
        };
